...
function friendlySyncError(error) {
  if (error instanceof CalDavError) {
    ...
    if (error.status === 403) {
      return 'iCloud 拒绝了操作（403）：Apple 通常要求使用邮箱形式的 Apple ID 才能开放 CalDAV 访问。请先确认 iPhone 已开启 iCloud 日历和提醒事项；若 Apple ID 只有手机号，请在 appleid.apple.com 查看或添加 iCloud 邮箱，再重新生成 App 专用密码';
    }
...
async function probeAuth(settings) {
  if (!settings.apple_id || !settings.app_password) {
    return { status: 'skipped', ok: false, message: '未配置 Apple ID 和 App 专用密码' };
  }
  try {
    const principal = await findPrincipal(settings);
    return { status: 207, ok: true, message: '认证成功，已发现 iCloud 账号' };
  } catch (error) {
    const status = error instanceof CalDavError ? error.status : 'error';
    return {
      status,
      ok: false,
...
async function diagnoseConnection(settings) {
  ...
  const auth = await probeAuth(settings || {});
  const message = ok
    ? '网络连接正常，可以访问 iCloud'
    : '网络连接异常。请检查电脑网络、防火墙，或改用非受限终端启动服务';
  return { host, port, ok, message, dns: dnsResult, tcp: tcpResult, https: httpsResult, auth };
}
