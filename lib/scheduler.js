...
function parseDurationFromTitle(title, fallbackDuration = 30) {
  ...
  const patterns = [
    ...
    { regex: /([一二两三四五六七八九十\d]+)\s*(?:页|頁)/, unit: 'pages' },
    { regex: /([一二两三四五六七八九十\d]+)\s*(?:题)/, unit: 'questions' },
    { regex: /([一二两三四五六七八九十\d]+)\s*(?:章)/, unit: 'chapters' },
    { regex: /([一二两三四五六七八九十\d]+)\s*(?:篇)/, unit: 'articles' },
  ...
  for (const pattern of patterns) {
    ...
    if (pattern.unit === 'pages') {
      return { duration: Math.max(10, Math.min(240, Math.round(value))), source: 'title' };
    }
    if (pattern.unit === 'questions') {
      return { duration: Math.max(10, Math.min(240, Math.round(value))), source: 'title' };
    }
    if (pattern.unit === 'chapters') {
      return { duration: Math.max(20, Math.min(300, Math.round(value * 30))), source: 'title' };
    }
    if (pattern.unit === 'articles') {
      return { duration: Math.max(15, Math.min(240, Math.round(value * 20))), source: 'title' };
    }
  }
...
function timeWindowFor(rule, category) {
  if (rule && Number.isFinite(Number(rule.start_min)) && Number.isFinite(Number(rule.end_min))) {
    return { start: Number(rule.start_min), end: Number(rule.end_min), label: '自定义', id: 'custom' };
  }
  ...
  return { start: block.start, end: block.end, label: block.label, id: block.id };
}
...
function consumeSegments(segments, start, end) {
  for (let i = segments.length - 1; i >= 0; i -= 1) {
    const segment = segments[i];
    if (end <= segment.start || start >= segment.end) continue;
    if (start <= segment.start && end >= segment.end) {
      ...
      const right = { start: end, end: segment.end };
      segment.end = start;
      segments.splice(i + 1, 0, right);
    }
  }
}
...
function scheduleDay(tasks, rules, { regenerate = false, settings = {} } = {}) {
  ...
  for (const task of movable) {
    ...
    const windowOrder = orderedWindowsFor(task);
    let placed = null;
    for (const window of windowOrder) {
      const preferred = intersectSegments(fullSegments, window.start, window.end);
      placed = placeInSegments(preferred, task.duration);
      if (placed) {
        consumeSegments(fullSegments, placed.start, placed.end);
        break;
      }
    }
    if (!placed) {
      placed = placeInSegments(fullSegments, task.duration);
      if (placed) consumeSegments(fullSegments, placed.start, placed.end);
    }
...
function orderedWindowsFor(task) {
  const estimateWindow = task.estimate.window;
  const priority = task.priority;
  const blocks = TIME_BLOCKS;
  const morning = blocks.find((block) => block.id === 'morning');
  const afternoon = blocks.find((block) => block.id === 'afternoon');
  const evening = blocks.find((block) => block.id === 'evening');
  if (priority === 'P1') {
    return [estimateWindow, morning, afternoon, evening].filter(uniqueWindow);
  }
  if (priority === 'P3') {
    return [estimateWindow, evening, afternoon, morning].filter(uniqueWindow);
  }
  return [estimateWindow, afternoon, morning, evening].filter(uniqueWindow);
}

function uniqueWindow(window, index, array) {
  return array.findIndex((item) => item && item.id === window.id) === index;
}
